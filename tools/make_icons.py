"""App 圖示：米色底、墨色宋體「旅行」，下緣一條朱赭色的路線與站點。
2026-09-25 使用者指定：圖示用宋體寫「旅行」（原本是明朝體「旅」＋「たび」標音）。
iOS 會自己切圓角，所以輸出滿版方形；maskable 版把內容縮進安全區。
字型用 macOS 內建的 Songti TC Bold（只用來畫圖示，不隨 App 發佈）；主畫面只有約 60pt，粗體在小尺寸比標準體清楚。"""
...
FONT_INDEX = 2        # Songti.ttc：2＝Songti TC Bold、7＝Songti TC Regular、5＝Songti TC Light
TEXT = "旅行"
