// 配色試看（暫時）：網址後面加 #/look 開啟，在真正的 App 上切換 6 種白底色塊的做法；使用者選定後會拿掉。
...
function apply(k) {
  sessionStorage.setItem('tk-look', k);
  const dark = isDark();
...
export function startLooks() {
  ...
  bar.addEventListener('click', (e) => {
    ...
    if (b.hasAttribute('data-look-close')) {
      sessionStorage.removeItem('tk-look');
  ...
  matchMedia('(prefers-color-scheme: dark)').addEventListener('change', () => bar && apply(sessionStorage.getItem('tk-look') || '3'));
  apply(sessionStorage.getItem('tk-look') || '3');
}
